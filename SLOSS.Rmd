...
mdl1 <- sloss_sim %>% filter(!Migration0_at_deforestation & habitat_patch_size!=100 & birth_rate_birds == 2 ) %>%  
  group_by(habitat_patch_size,birds_dispersal_distance, birth_rate_birds,birds_behavior) %>%
  summarise(across(birds_at_deforestation:count_birds,list(mean=mean=sd))) %>% mutate(birth_rate_birds = factor(birth_rate_birds)  )

mdl1 %>% mutate(birds_dispersal_distance=factor(birds_dispersal_distance),lb=calc_number_of_species_mean - calc_number_of_species_sd, ub=calc_number_of_species_mean + calc_number_of_species_sd,) %>% ggplot(aes(x=habitat_patch_size,y=calc_number_of_species_mean,color=birds_dispersal_distance)) + geom_point(position=position_dodge(width=2)) +  
  geom_errorbar(
        aes(ymin = lb, ymax = ub),
        width = 0.1,
        linetype = "dotted",
        position=position_dodge(width=2)) +
  scale_color_viridis_d(name="Dispersal") + theme_bw()+ facet_wrap( ~birds_behavior ) + ylab("Number of species") + xlab("Patch Size") + ggtitle("Lambda=2 Degraded 60% Migration")
